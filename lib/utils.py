...
def init_network_weights(net, mean=0, std = 0.1):
...
def get_next_batch(dataloader,device):
	...
	#batch_dict["dims"] = data_dict["dims"].to(device)
	#batch_dict["beta"] = data_dict["beta"].to(device)
...
def get_ckpt_model(ckpt_path, model, device):
	...
	# Iterate through the module names and their corresponding tensors
	for name, param in state_dict.items():
		print(f"Module Name: {name}, Shape: {param.shape}") 
...
def test_data_nn(model, pred_length, condition_length, dataloader,device,args,kl_coef):


	encoder, decoder, graph, digit_label, num_batch, _ = dataloader.load_data(data_type="test")
	...
	with torch.no_grad():
		for _ in tqdm(range(num_batch)):
			...
			batch_dict_digit = get_next_batch_new(digit_label, device)
			batch_dict_decoder = get_next_batch(decoder, device)

			results = model.compute_all_losses(batch_dict_encoder, batch_dict_decoder, batch_dict_graph,batch_dict_digit,args.num_atoms,
